...
namespace Drupal\ibase_media_embed_mixcloud\Commands;

use Consolidation\OutputFormatters\StructuredData\RowsOfFields;
use Drupal\Core\Batch\BatchBuilder;
use Drupal\Core\StringTranslation\StringTranslationTrait;
use Drupal\drush9_batch_processing\BatchService;
...
class IbaseMediaEmbedMixcloudCommands extends DrushCommands {
      ...
      if ($nid) {
        if ($channel = $this->mixcloud->getValidChannel($nid)) {
          ...
          $batchBuilder
            ...
            ->setFinishCallback([EmbedMixcloudService::class, 'batchProcessEpisodesFinished'])
            ->setErrorMessage(t('Batch has encountered an error.'));

          foreach ($channel_episode_endpoints as $i => $endpoint) {
            if ($i > 0) continue;
            $batchId = $i + 1;
            $batchBuilder->addOperation([EmbedMixcloudService::class, 'batchProcessEpisodeEndpoint'], [
              $endpoint,
              $batchId,
              $channel->id(),
            ]);
